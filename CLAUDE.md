...
- **`RAG_PROVENANCE_QA` — ✅ SHIPPED (2026-09-09).** Ciclo SDD 0–4 completo (Brainstorm→Define→Design→Build→`/verify-spec` OVERALL PASS WITH FINDINGS (~0.9)→Ship). Primeira capability de RAG do projeto (`EPIC-024`/`SPEC-018`/`ADR-026`), 0% construído até esta fatia. Em vez de documentos legais/fiscais novos, ancorada no que o projeto já tem de real: corpus de 11 parágrafos de metodologia curados manualmente (1 por `metric_id` já real — dívida, fiscal ×3, INSS ×2, macro ×5), `source_url` buscado de `metric_provenance` no momento da carga (nunca hardcoded — algumas fontes republicam sob nome de arquivo novo todo mês). Novo endpoint `POST /v1/knowledge/ask`: retrieval híbrida 100% dentro do BigQuery (`VECTOR_SEARCH` sem `CREATE VECTOR INDEX` — corpus pequeno, GA sem índice — + filtro de metadata; `SEARCH()`/full-text conscientemente fora de escopo, decisão rastreada em `ADR-061`) + gate de evidência 100% determinístico (`ADR-013`: só sintetiza resposta quando alguma nota recuperada tem distância de cosseno ≤ 0.45, limiar calibrado ao vivo contra o corpus real) + síntese via Gemini (`google-genai`, **1ª chamada de IA generativa do projeto em produção**), restrita ao contexto já validado pelo gate. Resposta não usa `ADR-028` (`observed`/`estimated`/`simulated`) — envelope próprio (`answer`/`citations[]`/`evidence_sufficient`), decisão registrada em `ADR-061` D2.
  - **2 bugs reais de produção, ambos corrigidos no mesmo dia:** (1) `refresh_embeddings()` filtrava `WHERE embedding IS NULL` — testado ao vivo antes de escrever o resto do pipeline: BigQuery armazena `ARRAY<FLOAT64> NULL` como array vazio (`[]`), nunca `NULL` de verdade; corrigido para `ARRAY_LENGTH(embedding) = 0 OR ... IS NULL`. (2) 1ª chamada real de `POST /v1/knowledge/ask` em produção 500ou — `roles/aiplatform.user` não bastava; faltava `bigquery.connections.use` do `api-runtime` na conexão `rag-vertex-ai` (3ª identidade de IAM distinta desta integração: SA da conexão → Vertex AI; `api-runtime` → Gemini direto; `api-runtime` → usar a conexão — cada uma seu próprio grant).
  - `/verify-spec` independente (subagente `general-purpose`, sessão nova, read-only) = OVERALL PASS WITH FINDINGS (~0.9) — 7/7 AT verificados ao vivo em produção (incluindo o caso negativo: pergunta fora do corpus retorna "evidência insuficiente" sem nunca chamar o LLM); gate determinístico confirmado estruturalmente no código, não só por afirmação; ambos os bugs relatados confirmados precisos contra código/Terraform real. 1 achado real corrigido antes do ship: requisito MUST de retrieval híbrida (`DEFINE` G4) incluía `SEARCH()`/full-text, silenciosamente reduzido a só vetor+metadata durante o build sem decisão rastreada — corrigido com nota explícita em `ADR-061`/`BUILD_REPORT`.
  - Arquivo: `.claude/sdd/archive/RAG_PROVENANCE_QA/`. PRs `#43`-`#49` (infra, corpus/embeddings, retrieval/endpoint, hotfix IAM, docs, achado verify-spec, ship). ADR-061; código em `ingestion/{data/rag_knowledge_corpus.yaml,src/ingestion/rag_corpus.py,scripts/load_rag_corpus.py}`, `api/src/api/knowledge.py`, `infra/terraform/{bigquery_connection_vertex.tf,cloud_run_services.tf}`.
- **`LANDING_PAGE_TABS` — ✅ SHIPPED (2026-09-09).** Ciclo SDD 0–4 completo (Brainstorm→Define→Design→Build→`/verify-spec` OVERALL PASS (~0.95)→Ship). Pedido explícito do usuário: leu `Brasil_2036.pdf` (documento de contextualização fornecido pelo usuário, não commitado — mesmo padrão do `docs/landpage.png`) e reestruturou a landing (`LANDING_PAGE_ASTRO`, scroll único) em 5 abas client-side (WAI-ARIA APG, `Nav.astro` já sticky virou a própria `tablist`, hash sincronizado): **Home** (proposta + `FiscalSnapshot.astro` novo — 7 indicadores fiscais/macro reais da p.2 do PDF, PIB/IPCA/Dívida Bruta-PIB 2026 e 2027/déficit primário/receita líquida/despesa total, cada um citando a fonte exata do PDF — SPE/MF Boletim Macrofiscal jul/2026 ou Prisma Fiscal ago/2026); **Dados Reais** (`DataPanel`, intocado — zero-diff confirmado pelo `/verify-spec`); **Módulos & Portais** (+3 cenários de referência do PDF); **Arquitetura & Roadmap** (+acessos/governança, evolução da landing, questão central); **Sobre** (+expectativas).
  - **Achado real que moldou toda a descoberta:** os 7 números fiscais do PDF (Boletim Macrofiscal/Prisma Fiscal do MF) são reais, mas de fonte diferente do que o pipeline do projeto já ingere (Tesouro/BCB SGS). Resolvido explicitamente com o usuário antes de codar: os 2 conjuntos nunca se misturam na mesma UI — `FiscalSnapshot` visualmente distinto de `DataPanel`, sem `data-evidence` (não é claim de status de entrega), `DataPanel` permanece 100% intocado (`ADR-012`).
  - `/verify-spec` independente (subagente `general-purpose`, sessão nova, read-only) = OVERALL PASS (~0.95) — 8/8 AT PASS ao vivo em produção; os 7 valores fiscais conferidos um a um contra o PDF real, zero discrepância; `DataPanel.astro` confirmado zero-diff. 1 observação não-bloqueante pré-existente (e2e de `web/` não é gate de CI, não introduzida por esta fatia).
  - Arquivo: `.claude/sdd/archive/LANDING_PAGE_TABS/`. PR `#51` (ship em `#52`). Código em `web/src/components/{FiscalSnapshot,Scenarios,AccessGovernance,LandingEvolution,Expectations,Nav,FooterCta}.astro`, `web/src/pages/index.astro`, `web/tests/e2e/{card,tabs}.spec.ts`.
- **Follow-ups rastreados (SHIPPED §7 de CI_ASSURANCE_GATES):** revisão humana obrigatória em `main` (hoje só CI); job noturno contra a fonte real do Tesouro (drift); bump de actions Node 20. **Follow-ups antigos ainda abertos (MVP_WALKING_SKELETON §7):** URL do catálogo dados.gov.br → `dataset_registry.source_url` (concurso CGU); `wif.tf`/serviços no Terraform; `MANIFEST.json`; provenance histórica. **Follow-ups do INSS_BENEFICIOS:** parser adaptativo por nome de coluna para backfill completo de Emitidos/Mantidos; Mantidos sem dado real carregado; W1 (RAW do Indeferidos não preserva XLSX original) — baixa prioridade. **Follow-up do FISCAL_RECEITA_DESPESA:** e2e do módulo fiscal com asserção fraca (SHIPPED §7) — baixa prioridade. **Follow-ups do BIGQUERY_OPERATIONAL_STANDARDS (SHIPPED §7):** `ingestion/scripts/*.py` sem cobertura de teste unitário (só verificação por execução real — já deixou 1 bug real escapar até produção); orçamento de projeto/billing export (`R-012`, `EPIC-042`, ainda não implementado); TTL de Bronze/Silver (`SPEC-034 §3`, decisão explícita de não implementar ainda — YAGNI); enforcement automático via lint de CI. **Follow-ups do LANDING_PAGE_ASTRO (SHIPPED §7):** comentários de graceful-degradation em `card.spec.ts` desatualizados (pré-existente); `status.ts` exige atualização manual a cada `/ship` futuro, sem enforcement automático. **Follow-ups do DEBTLAB_SIMULATOR (SHIPPED §7):** `pib_mensal`/`divida_bruta_pib` não plugados no job noturno `data.yml` (mesmo padrão de débito já rastreado); sem UI de simulador em `web/` ainda; rate-limiting real do endpoint de escrita fica pra quando RBAC/ABAC existir. **Follow-ups do MACRO_TWIN_EXPANSION:** `ipca_mensal`/`selic_mensal`/`cambio_usd_brl` não plugados no job noturno `data.yml` (mesmo padrão já rastreado); Macro Twin ainda sem forecast/cenários completos (`EPIC-008`); sem UI de `suggested-assumptions` em `web/` ainda. **Follow-ups do RAG_PROVENANCE_QA:** `SEARCH()`/full-text não implementado, reavaliar junto com `CREATE VECTOR INDEX` se o corpus crescer (`ADR-061`); `rag_knowledge_corpus` não plugado no job noturno `data.yml`; sem UI de Q&A em `web/` ainda. **Follow-ups do LANDING_PAGE_TABS:** e2e de `web/` (`tabs.spec.ts`/`card.spec.ts`) não é gate de CI (`web-check` só roda `typecheck`+`build`) — pré-existente, sem enforcement automático; conectar os 7 cards fiscais a uma fonte real ingerida (Boletim Macrofiscal/Prisma Fiscal) fica pra uma fatia futura de dado, se pedida.
- **Próximo passo:** iniciar a próxima feature via `/brainstorm` ou `/define` — sequência já combinada com o usuário: RBAC/ABAC + portal autenticado (RAG básico e reestruturação da landing em abas concluídos).
