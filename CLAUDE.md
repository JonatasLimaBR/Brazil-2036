...
- **`INSS_BENEFICIOS` — 🔶 backfill real rodado e escopo fechado (2026-09-05), falta só `/ship`.** Fatia #2: ingestão dos 3 datasets P0 de INSS (Emitidos/Mantidos/Indeferidos, `SPEC-011`/`PRD-005`/`EPIC-010`). Descoberta real (`dadosabertos.inss.gov.br`, não `dados.gov.br`) revelou microdado por evento — 2-3 ordens de grandeza maior que a dívida — exigindo escrita por partição e backfill resumível (`ADR-055`). PR1 (espinha de dados) e PR2 (API+web) já mergeados em 2026-09-04. **Corrigiu um risco crítico pré-existente** antes de tocar produção: `registry.py`/`provenance.py` faziam `CREATE OR REPLACE TABLE` em tabelas *compartilhadas* (`dataset_registry`, `metric_provenance`) — rodar para um 2º dataset teria apagado os dados da dívida.
  - **Backfill real contra `brasil2036-dev` (confirmado pelo usuário a cada etapa):** Indeferidos completo — 37/38 meses, 15.142 linhas, XLSX→CSV via openpyxl. Emitidos — apenas 1 mês no formato atual (2026-06/07), 1.216 linhas; os demais ~36 meses ficaram de fora porque o formato-fonte mudou pelo menos 4 vezes desde 2023 (não é um simples corte histórico-vs-atual). Mantidos — nenhum mês rodado. **Decisão explícita do usuário: aceitar como está e fechar o escopo do backfill aqui** — não perseguir mais histórico nem parser adaptativo nesta sessão.
  - **`/verify-spec` independente (sessão nova, read-only) = OVERALL PASS**, verificado ao vivo contra BigQuery e endpoints em produção (não só documentos). Achado E1 (rota `/v1/metrics/inss_beneficios_mantidos/national` devolvia 500 em vez de 404, por `NotFound` não tratado — Mantidos nunca rodou) **corrigido** em `api/src/api/bigquery_repo.py`. Achado W1 (`InssIndeferidosConnector` não preserva os bytes originais do XLSX em RAW, desvio do DESIGN D6) **aceito como desvio documentado**, prioridade baixa.
  ...
  - **Pendente (único bloqueador para `/ship`):** merge da correção do E1 (PR em andamento) → `/ship`. Os endpoints `/national` de Mantidos (e a maior parte de Emitidos) continuam devolvendo 404 em produção — esperado dado o escopo fechado, não é bug.
- **Follow-ups rastreados (SHIPPED §7 de CI_ASSURANCE_GATES):** cap de bytes por query no gate `integration` (residual aceito, custo ínfimo); revisão humana obrigatória em `main` (hoje só CI); job noturno contra a fonte real do Tesouro (drift); bump de actions Node 20. **Follow-ups antigos ainda abertos (MVP_WALKING_SKELETON §7):** URL do catálogo dados.gov.br → `dataset_registry.source_url` (concurso CGU); `wif.tf`/serviços no Terraform; `MANIFEST.json`; provenance histórica. **Novo (INSS_BENEFICIOS):** W1 (RAW do Indeferidos não preserva XLSX original) — baixa prioridade; backfill completo de Emitidos/Mantidos exigiria parser adaptativo por schema — engenharia maior, deliberadamente fora deste ciclo.
- **Próximo passo:** mergear a correção do E1 → `/ship` de `INSS_BENEFICIOS` → iniciar próxima feature via `/brainstorm` ou `/define`.
