...
- **Próximo passo:** pedido do usuário — avaliar práticas de FinOps/DataOps para BigQuery (cap de bytes por query, SPEC/ADR de padrões de particionamento/clustering) via `/brainstorm`; MLOps/LLMOps/AgentOps deliberadamente fora de escopo por não haver modelo/agente em produção ainda.
