...
- **`INSS_BENEFICIOS` — 🔶 PR1 merged (2026-09-04), PR2 pendente.** Fatia #2: ingestão dos 3 datasets P0 de INSS (Emitidos/Mantidos/Indeferidos, `SPEC-011`/`PRD-005`/`EPIC-010`). Descoberta real (`dadosabertos.inss.gov.br`, não `dados.gov.br`) revelou microdado por evento — 2-3 ordens de grandeza maior que a dívida (0,8–1,2 GB/mês comprimido, 3 formatos de origem: ZIP/CSV misto/XLSX) — exigindo escrita por partição e backfill resumível (`ADR-055`). **PR1 (espinha de dados) provado ao vivo contra BigQuery real** (gate `integration`, 2 execuções verdes): `pipeline_incremental.py` novo (não toca `pipeline.run()` da dívida), `backfill.py` resumível, `ckan.py` (lister genérico), 3 conectores com schema real confirmado, 3 contratos, 6 SQL, 3 configs. **Corrigiu um risco crítico pré-existente** antes de tocar produção: `registry.upsert_dataset_registry()`/`provenance.write_from_gold()` faziam `CREATE OR REPLACE TABLE` em tabelas *compartilhadas* (`dataset_registry`, `metric_provenance`) — rodar para um 2º dataset teria apagado os dados da dívida.
  ...
  - **Pendente:** PR2 (extensão da API para grão mês+espécie + módulo M03 na Landing); backfill histórico completo (~183 recursos, ~100–130 GB) contra GCP real, ainda não disparado (operação longa/custosa, pede confirmação explícita antes de rodar); `/verify-spec` + `/ship` só depois dos dois acima.
...
- **Próximo passo:** continuar `INSS_BENEFICIOS` (PR2 → backfill real → `/verify-spec` → `/ship`) antes de iniciar qualquer feature nova.
