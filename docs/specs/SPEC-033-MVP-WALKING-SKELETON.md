...
`dados.gov.br` resource → GCS RAW (immutable) → BigQuery Bronze → Silver → Gold → one
canonical metric with a full provenance object → public Landing card.
Dataset: "Dívida Consolidada dos Estados e do DF". Runs in one GCP dev project only.
Delivered in two PRs (ADR-051, ADR-052 apply).
...
`.claude/sdd/features/DESIGN_MVP_WALKING_SKELETON.md`.

## MUST — PR1 (data spine)
- Register exactly one active row in `br2036_control.dataset_registry` for the resource
  (`br2036_domain='fiscal'`, `br2036_module='M02'`); discovery MUST NOT auto-promote (SPEC-002).
...
- Layers: `br2036_bronze.debt_state_raw` (source-shaped, STRING + `_source_uri`, `_ingested_at`,
  `_row_hash`); `br2036_silver.debt_state` (typed; entity → `state_ibge_code` via
  `br2036_control.ref_estado_ibge`; period → `reference_date DATE`; `value NUMERIC`, `unit='BRL'`);
  `br2036_gold.gold_debt_state_current` (`metric_id='divida_consolidada_liquida'`,
  `data_class='observed'`).
- An unmapped territorial entity fails the run; no silent drop (ADR-011).
- Data contract v1 (`ingestion/contracts/divida_consolidada_estados.yaml`, SPEC-005) is enforced
  ...
  alert — SPEC-004, ADR-052) and Gold (completeness, NOT NULL on keys, `value >= 0`, provenance
  coverage).
...
- FastAPI/Pydantic service exposes `GET /v1/metrics/{metric_id}` (with optional
  `state_ibge_code`) returning `value`, `unit`, `reference_date`, typed `data_class`
  (`observed|estimated|simulated`, ADR-028) and a provenance summary; and
...
- Public Landing renders one card: value, `reference_date` (latest published =
  `MAX(reference_date)`), a "fonte" link to the real `source_url`, and a visual `data_class`
...
- `infra/terraform/**`, `ingestion/**` (connector, RAW/Bronze/Silver/Gold, contract, provenance,
  pipeline, `reference/estado_ibge.csv`, tests), `api/**`, `web/**`.
...
- Full stale-source blocking (R-008), Data Trust Score (SPEC-006), semantic layer (SPEC-008),
  historical series on the card, INSS as slice #2.
